...
const Cart = async () => {
  ...
  return <>
    <section className="py-2">
      {!carts.length && <p className="text-center">Your cart is empty</p>}

      {!!carts.length && (
        <ul>
          {carts.map((item) =>
            <li key={item.book_id} className="flex gap-x-4">
              <Link href={`/books/${item.book_id}`} className="block relative aspect-[3/4] basis-1/3 md:basis-1/5 dark:bg-zinc-800 rounded-lg" >
                <Image priority fill src={item.books.cover} alt={`${item.books.name}'s cover'`} className="rounded-lg" />
              </Link>
              <div className="space-y-4">
                <Link href={`/books/${item.book_id}`}>
                  <h3 className="text-xl font-bold">{item.books.name}</h3>
                </Link>
                <QuantityButton bookId={item.book_id} initialQuantity={item.quantity} />
