...
function UserDropDown() {
  ...
  return <DropdownMenu>
    <DropdownMenuTrigger>
      <Avatar>
        <AvatarFallback className="uppercase">{!!user ? user.username.slice(0, 2) : "U"}</AvatarFallback>
      </Avatar>
    </DropdownMenuTrigger>
...
function UnauthorizedComponent() {
  return <DropdownMenuContent side="bottom">
...
function AuthorizedComponent({ user }: { user: User }) {
  return <DropdownMenuContent side="bottom" className="space-y-2">
    <DropdownMenuLabel className="text-xl">{user.username}</DropdownMenuLabel>
    <DropdownMenuItem asChild><Link href={"/profile"}>Profile</Link></DropdownMenuItem>
    <DropdownMenuSeparator />
    {user.permission === 'admin' &&
      <DropdownMenuItem asChild><Link href={'/upload'}>Upload</Link></DropdownMenuItem>
    }     <DropdownMenuSeparator />
    <DropdownMenuItem asChild><SignOut>Sign out</SignOut></DropdownMenuItem>
  </DropdownMenuContent>
}
