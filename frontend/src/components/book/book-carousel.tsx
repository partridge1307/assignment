    ...
    <CarouselContent>
      {books?.map(book =>
        <CarouselItem key={book.id} className="basis-full">
          <Link href={`/books/${book.id}`} className="flex flex-nowrap gap-x-3 md:gap-x-6 md:px-16 bg-gradient-to-br dark:from-zinc-900 rounded-lg hover:bg-zinc-900 transition duration-500">
            <div className="relative aspect-[3/4] basis-1/2 md:basis-1/4">
              <Image priority fill src={book.cover} alt="Carousel Cover" className="rounded-lg" />
            </div>
